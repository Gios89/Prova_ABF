...
function fill_Data(n::Int64, d::Int64)::Bool
    ...
    for v in 2:(d+1) #run the computation among all graphs with fixed number of vertices
        ...
        for n_g in 1:n_trees_nv  #run the computation for a fixed graph
            ...
            pruf_str0 = string(split(str, ',')[1],"0.clr")
            if !(pruf_str0 in readdir(Dim_dir))
                url = "https://raw.githubusercontent.com/Gios89/Colorations/main/Dimension$n/$pruf_str0"
                dest = Dim_dir*"/$pruf_str0"
                try
